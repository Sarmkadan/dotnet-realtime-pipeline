...
public static class BackpressureEventJsonExtensions
{
    ...
    /// <returns>The deserialized backpressure event, or null if the JSON is null or empty.</returns>
    ...
    public static BackpressureEvent? FromJson(string json)
    {
        ...
        return JsonSerializer.Deserialize<BackpressureEvent>(json, _jsonOptions);
    }
    ...
    /// </summary>
    /// <param name="json">The JSON string to deserialize.</param>
    /// <param name="value">Receives the deserialized backpressure event if successful.</param>
    ...
    public static bool TryFromJson(string json, out BackpressureEvent? value)
    {
        ...
        try
        {
            value = JsonSerializer.Deserialize<BackpressureEvent>(json, _jsonOptions);
            return true;
        }
        catch (JsonException)
        {
