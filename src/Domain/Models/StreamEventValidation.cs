...
public static class StreamEventValidation
{
	...
	public static IReadOnlyList<string> ValidateEvent(this StreamEvent value)
	{
		...
		if (value.EventId <= 0)
		{
			problems.Add("EventId must be positive.");
		}

		// Validate DataPointId
		if (value.DataPointId <= 0)
		{
			problems.Add("DataPointId must be positive.");
		}

		// Validate Timestamp
		if (value.Timestamp <= 0)
		{
			problems.Add("Timestamp must be positive.");
		}
		...
		{
			problems.Add(string.Create(CultureInfo.InvariantCulture, $"EventType exceeds maximum length of 100 characters (current: {value.EventType.Length})."));
		}

		// Validate Priority
		if (value.Priority is < 1 or > 10)
		{
			problems.Add(string.Create(CultureInfo.InvariantCulture, $"Priority must be between 1 and 10 inclusive (current: {value.Priority})."));
		}

		// Validate SourceSystem
		if (value.SourceSystem?.Length > 100)
		{
			problems.Add(string.Create(CultureInfo.InvariantCulture, $"SourceSystem exceeds maximum length of 100 characters (current: {value.SourceSystem.Length})."));
		}

		// Validate CorrelationId
		if (value.CorrelationId?.Length > 100)
		{
			problems.Add(string.Create(CultureInfo.InvariantCulture, $"CorrelationId exceeds maximum length of 100 characters (current: {value.CorrelationId.Length})."));
		}

		// Validate CausationId
		if (value.CausationId?.Length > 100)
		{
			problems.Add(string.Create(CultureInfo.InvariantCulture, $"CausationId exceeds maximum length of 100 characters (current: {value.CausationId.Length})."));
		}
		...
		{
			problems.Add(string.Create(CultureInfo.InvariantCulture, $"Payload dictionary exceeds maximum size of 1000 items (current: {value.Payload.Count})."));
		}
		...
		else
		{
			...
			{
				problems.Add(string.Create(CultureInfo.InvariantCulture, $"ProcessedByStages list exceeds maximum size of 50 items (current: {value.ProcessedByStages.Count})."));
			}
			...
			foreach (var stage in value.ProcessedByStages)
			{
				...
				if (stage.Length > 100)
				{
					problems.Add(string.Create(CultureInfo.InvariantCulture, $"ProcessedByStages entry exceeds maximum length of 100 characters (current: {stage.Length})."));
					break;
				}
			}
		}
		...
		{
			problems.Add(string.Create(CultureInfo.InvariantCulture, $"RetryAttempt exceeds maximum of 100 (current: {value.RetryAttempt})."));
		}
		...
		else if (value.LastErrorMessage?.Length > 1000)
		{
			problems.Add(string.Create(CultureInfo.InvariantCulture, $"LastErrorMessage exceeds maximum length of 1000 characters (current: {value.LastErrorMessage.Length})."));
		}
